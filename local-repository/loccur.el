...
;; This is version 1.2.5 plus my custom user option `loccur-prompt-current-word'.
...
(defcustom loccur-prompt-current-word t
  "If set to nil, prompt with an empty string, and access the word at
point with \\<minibuffer-mode-map>\\[next-history-element] instead."
  :type '(boolean))
...
REGEX is regexp to search"
  (interactive
   (if loccur-mode
       nil
     (let ((prompt (loccur-prompt)))
       (if loccur-prompt-current-word
           (list (read-string "Loccur: " prompt 'loccur-history ""))
         (list (read-string "Loccur: " nil 'loccur-history prompt))))))
