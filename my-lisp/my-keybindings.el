...
(let ((keymap my-keys-local-minor-mode-map))
  ...
  (define-key keymap (kbd "C-x v C-s") 'vc-log-search)
  ...
  (define-key keymap (kbd "<menu> t")  'my-pomodoro) ;; t for timer
  ...
  (define-key keymap (kbd "C-z C-f")   'my-project-find-file)
  (define-key keymap (kbd "C-c m m")   'magit-status)
  (define-key keymap (kbd "C-c m M")   'magit-dispatch)
  (define-key keymap (kbd "C-c m ?")   'magit-dispatch)
  (define-key keymap (kbd "C-c m b")   'mo-git-blame-current)
  (define-key keymap (kbd "C-c m d")   'magit-diff)
  (define-key keymap (kbd "C-c m l")   'magit-log)
  (define-key keymap (kbd "C-c m f")   'magit-file-dispatch)
  ...
  (define-key keymap (kbd "C-h M-k")   'describe-keymap)
  ...
  (define-key keymap (kbd "<f6>")      'my-rgrep) ;; ripgrep-regexp
...
(defun my-describe-keymap (keymap)
  ...
  (interactive
   (list (let ((sym (symbol-at-point)))
           (unless (and (boundp sym)
                        (keymapp (symbol-value sym)))
             (setq sym nil))
           (completing-read
            (if sym (format "Keymap (default %s): " sym) "Keymap: ")
            (let (maps)
              (mapatoms (lambda (sym)
                          (and (boundp sym)
                               (keymapp (symbol-value sym))
