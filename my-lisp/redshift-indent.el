...
;; FIXME: Something I've done has broken the ability of TAB to indent
;; to the correct column *regardless* of whether the mode is enabled
;; or not :( e.g.: In the variable definition for `hl-sexp-face', the
;; line "(:bold t))" moves to a different (relative) column when I press
;; TAB, depending on whether or not `redshift-indent-mode' is enabled.
;; redshift-indent_2.el (0.28) certainly doesn't have this problem.
;;
;; It appears to be necessary that we use the :relative-width
;; specification rather than :width, otherwise the above issue
;; occurs. TODO: Investigate what (calculate-lisp-indent) does
;; differently in each circumstance. (:bold t)) is indented 5
;; normally and with :relative-width, but 7 when :width is used.
;;
;; C-x C-f ~/.emacs.d/my-lisp/hl-sexp.el
...
;; visual alignments are prone to break with this mode enabled.) You can
;; customize the `redshift-regexp' variable, and/or write a custom function
;; for the `redshift-search-forward' variable, in order to achieve more
;; intelligent behaviour if desired.
;;
...
(defun redshift--add-text-properties (beginning end width)
  ;; FIXME: Depending on how this code finishes up, the WIDTH arg may
  ;; end up being redundant, in which case it should be removed.
  ;;
