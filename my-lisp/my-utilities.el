...
(defvar reminder-frame-mode-map
  ;; Only allow 'q' (`View-quit') to dismiss the reminder frame.
  ;;
  ;; More probably, I shouldn't be using `view-mode' at all?! although
  ;; for a large message there's conceivable benefit in having some of
  ;; those commands available.
  ;;
  ;; At minimum, though, I don't want any of the following commands to
  ;; function because I can too easily be typing those bindings at the
  ;; time the buffer appears (and if I actually had cause to edit the
  ;; buffer, "C-x C-q" will do the job).  Mind you, I might also have
  ;; just typed a "q", so... maybe there should be an in-built delay?
  ;; TODO: implement that delay.
  (let ((map (make-sparse-keymap)))
    (define-key map [remap View-exit] #'my-interactive-ding)
    (define-key map [remap View-exit-and-edit] #'my-interactive-ding)
    (define-key map [remap View-leave] #'my-interactive-ding)
    (define-key map [remap View-quit-all] #'my-interactive-ding)
    (define-key map [remap View-kill-and-leave] #'my-interactive-ding)
    map)
  "Keymap for `reminder-frame-mode'.")
